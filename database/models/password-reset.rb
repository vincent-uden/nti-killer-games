...
class MailHandler
  ...
  def self.send_reset_email(token, user_id)
    ...
    Pony.mail(
      ...
      body: "<h1>Återställ ditt lösenord</h1><p>Hej #{user.get_first_name}!<br>Vi har nyligen fått en ansökan om att återställa lösenordet på ditt konto. Om detta var du följer du länken nedan för att välja ett nytt lösenord. Annars borde du dubbelkolla säkerheten på ditt mail-konto och radera detta mail.</p><a href=\"https://#{$domain_name}/account/newpw?token=#{token}&email=#{user.get_email}\">https://#{$domain_name}/account/newpw?token=#{token}&email=#{user.get_email}</a>"
    )
